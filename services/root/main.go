// Command root is a practice-compatible stand-in for the "root" server
...
// exercise the infrastructure participants build (AppConfig, Redis,
// filesystem/EFS mount, S3). Business logic and exact response bodies are
// illustrative, not a reverse-engineering of the proprietary original.
package main
...
const usage = `root - Unicorn Service "root" practice server
...
CONFIGURATION:
  ...
  from AppConfig. If those are not all set, root falls back to reading a
...
DEPENDENCIES:
  ...
  - Redis (ElastiCache), non-cluster mode, for response caching.
...
ENDPOINTS:
  ...
  GET  /unicorns/{id}   cache-through demo across Redis and FsPath
...
func main() {
	...
	var err error
	if ids.complete() {
		log.Printf("root: fetching configuration from AWS AppConfig (application=%s environment=%s profile=%s)",
			ids.Application, ids.Environment, ids.Profile)
		cfg, err = LoadConfigFromAppConfig(context.Background(), ids)
	} else {
		log.Printf("root: --appconfig-* flags not fully set, falling back to local config file %q", *configPath)
	...
	server := NewServer(cfg, cache, storage)

	addr := fmt.Sprintf(":%d", cfg.Port)
	log.Printf("root: listening on %s (fsPath=%s bucket=%s redisHost=%s)",
		addr, cfg.FsPath, cfg.Bucket, cfg.RedisHost)
